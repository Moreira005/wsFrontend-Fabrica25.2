...
// ELEMENTOS DO DOM
const containerPokemons = document.getElementById(
  "pokemonContainer"
) as HTMLElement;
const inputBusca = document.getElementById("procurarInput") as HTMLInputElement;
const formularioBusca = document.getElementById(
  "procurarPokemon"
) as HTMLFormElement;
const botaoAlternar = document.getElementById(
  "botaoAlternar"
) as HTMLButtonElement;

// "BANCO DE DADOS" LOCAL
let todosOsPokemons: Pokemon[] = [];
let modoVisualizacao: "grade" | "lista" = "grade";

// FUNÇÕES

function renderizarPokemons(listaDePokemons: Pokemon[]) {
...
function criarCardPokemon(pokemon: Pokemon): string {
  ...
  const htmlDoCard = `
    <article class="pokemonCard" onclick="window.location.href='detalhes.html?id=${
      pokemon.id
    }'">
      <figure class="pokemonImagemCard">
        <img src="${urlDaImagem}" alt="Imagem do Pokémon ${capitalizar(
    pokemon.nome
  )}" loading="lazy" />
      </figure>
      <div class="pokemonInformacaoCard">
        <span class="pokemonIdCard">#${String(pokemon.id).padStart(
          3,
          "0"
        )}</span>
...
async function buscarTodosOsPokemons() {
  try {
    const resposta = await fetch(
      "https://pokeapi.co/api/v2/pokemon?limit=1161"
    );
    const dados = await resposta.json();

    // Usando 'nome: p.name' para criar a propriedade 'nome'
    todosOsPokemons = dados.results.map((p: { name: string; url: string }) => {
...
function manipularBusca() {
  ...
  const pokemonsFiltrados = todosOsPokemons.filter((pokemon) =>
    pokemon.nome.toLowerCase().includes(termoDeBusca)
  );
